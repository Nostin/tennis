...
def extract_match_odds(file_path, processed_markets):
                ...
                try:
                    ...
                    for market in data["mc"]:
                        ...
                        if "marketDefinition" in market:
                            ...
                            # Create a unique market identifier using event_id and market_time
                            market_key = f"{event_id}_{market_time.strftime('%Y%m%d%H%M%S')}"
    ...
    # Mark processed markets using the unique market_key
    for market_id, data in market_data.items():
        processed_markets.add(data["market_key"])
...
def process_all_years(base_directory):
    """Process all years of data, creating a separate CSV file for each year."""
    year_stats = {}  # Track statistics for each year
    processed_markets = set()  # Track which markets we've already processed
    ...
    for year in os.listdir(base_directory):
                ...
                for match_id in os.listdir(day_path):
                    ...
                    for filename in os.listdir(match_path):
                        if filename.endswith(".bz2"):
                            ...
                            market_data = extract_match_odds(file_path, processed_markets)
        ...
        # Save year's data to a separate CSV file
        if all_odds_records:
            output_file = f"betfair_odds_{year}.csv"
            ...
            year_stats[year] = {
                ...
                "total_records": len(all_odds_records),
                "duplicate_markets": duplicate_markets
            }
        else:
            logging.warning(f"No records were extracted for year {year}")

    # Print summary statistics
    logging.info("\nYearly Processing Summary:")
    for year, stats in year_stats.items():
        logging.info(f"Year {year}:")
        logging.info(f"  Processed files: {stats['processed_files']}")
        logging.info(f"  Skipped files: {stats['skipped_files']}")
        logging.info(f"  Total records: {stats['total_records']}")
        logging.info(f"  Duplicate markets skipped: {stats['duplicate_markets']}")

# Run extraction for all years
process_all_years(BASE_DIRECTORY)
